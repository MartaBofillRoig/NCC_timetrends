...
data_sim_block <- function(K=1, mu0=0, delta, p0, OR, lambda, sigma, N1, N2, N_add, N_peak, trend, trend_param, endpoint){
  ...
  if(endpoint=="binary"){
    ...
    # p0_stage2 <- Data[Data$treatment==0 & Data$stage==2,]$p[1]
    # p2_stage2 <- Data[Data$treatment==2 & Data$stage==2,]$p[1]
    # 
    # Data$OR_inf2 <- (p2_stage2/(1-p2_stage2))/(p0_stage2/(1-p0_stage2))
