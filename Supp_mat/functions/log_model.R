...
logistic_model_a2_int <- function(data, OR, alpha=0.025){
  ...
  # fit logistic model
  mod <- glm(response ~ as.factor(treatment) + as.factor(stage) + trt_1:stage_2, data, family = "binomial")
  res <- summary(mod)
...
# Logistic model separate

logistic_model_sep <- function(data, OR, alpha=0.025){
  
  data <- data[data$treatment %in% c(0, 2) & data$stage %in% c(2, 3),]
